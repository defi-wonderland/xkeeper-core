...
contract AutomationVaultForTest is AutomationVault {
  ...
  function addRelayForTest(
    ...
    address _job,
    SelectorData[] memory _selectorData
  ) public {
    ...
    for (uint256 _i; _i < _selectorData.length; ++_i) {
      _approvedJobSelectorsList[_relay][_job].add(bytes32(_selectorData[_i].selector));
      _approvedJobSelectorsWithHooks[_relay][_job][_selectorData[_i].selector] = _selectorData[_i].hookData;
    }
...
abstract contract AutomationVaultUnitTest is Test {
  ...
  function _assumeRelayData(
    ...
    vm.assume(_selectorsData.length > 0 && _selectorsData.length < 30);
  }
}
...
contract UnitGetRelayData is AutomationVaultUnitTest {
  ...
  modifier happyPath(
    ...
    _assumeRelayData(_relay, _callers, _job, _selectorsData);
    ...
    // Clean the array to avoid duplicates
    for (uint256 _i; _i < _selectorsData.length; ++_i) {
      _cleanSelectors.add(_selectorsData[_i].selector[_i]);
    }

    automationVault.addRelayForTest(_relay, _callers, _job, _selectorsData);
  ...
  function testRelayData(
    ...
    address _job,
    IAutomationVault.SelectorData[] memory _selectorsData
  ) public happyPath(_relay, _callers, _job, _selectorsData) {
